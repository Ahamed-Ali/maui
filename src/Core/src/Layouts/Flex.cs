...
namespace Microsoft.Maui.Layouts.Flex
{
	...
	class Item : List<Item>
	{
		...
		public void Layout(bool isMeasuring = true)
		{
			if (Parent != null)
			...
			layout_item(this, Width, Height, isMeasuring);
		}

		public delegate void SelfSizingDelegate(Item item, ref float width, ref float height, bool isMeasuring = true);
		...
		static void layout_item(Item item, float width, float height, bool isMeasuring)
		{
			...
			for (int i = 0; i < item.Count; i++)
			{
				...
				if (child.Position == Position.Absolute)
				{
					...
					layout_item(child, child.Frame[2], child.Frame[3],isMeasuring);
					continue;
				}
				...
				if (child.SelfSizing != null)
				{
					...
					child.SelfSizing(child, ref size[0], ref size[1], isMeasuring);
				...
				if (layout.wrap)
				{
					if (layout.flex_dim < child_size)
					{
						...
						layout_items(item, last_layout_child, i, relative_children_count, ref layout, isMeasuring);
			...
			layout_items(item, last_layout_child, item.Count, relative_children_count, ref layout, isMeasuring);
		...
		static void layout_items(Item item, int child_begin, int child_end, int children_count, ref flex_layout layout, bool isMeasuring)
		{
